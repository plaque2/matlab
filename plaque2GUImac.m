function varargout = plaque2GUImac(varargin)
% PLAQUE2GUIMAC MATLAB code for plaque2GUImac.fig
%      PLAQUE2GUIMAC, by itself, creates a new PLAQUE2GUIMAC or raises the existing
%      singleton*.
%
%      H = PLAQUE2GUIs returns the handle to a new PLAQUE2GUIMAC or the handle to
%      the existing singleton*.
%
%      PLAQUE2GUIMAC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PLAQUE2GUIMAC.M with the given input arguments.
%
%      PLAQUE2GUIMAC('Property','Value',...) creates a new PLAQUE2GUIMAC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before plaque2GUImac_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to plaque2GUImac_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help plaque2GUImac

% Last Modified by GUIDE v2.5 17-Nov-2014 19:25:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @plaque2GUImac_OpeningFcn, ...
    'gui_OutputFcn',  @plaque2GUImac_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
if isdeployed % Stand-alone mode.
    ...
    %[status, result] = system('pwd'); 
     str = which('logo.png');
    currentDir =str(1:end-9);%fullfile(ctfroot,'Plaque2'); %char(regexpi(result, 'Path=(.*?);', 'tokens', 'once'));
...
% --- Executes just before plaque2GUImac is made visible.
function plaque2GUImac_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to plaque2GUImac (see VARARGIN)
...
% Choose default command line output for plaque2GUImac
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes plaque2GUImac wait for user response (see UIRESUME)
...
function varargout = plaque2GUImac_OutputFcn(hObject, eventdata, handles)
