...
# User input
plaqueOutputFile <- 'N:/antivir_screen/6-prestwick/6-12_HRV_confirmation/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_24/Results/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_24_ImageData.csv'
layoutFile <- 'N:/antivir_screen/6-prestwick/6-12_HRV_confirmation/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_24/Layouts/BafA_p2_vHRV-GFP_d25e4.csv'
plateName = str_match(plaqueOutputFile, '(^.*/(.+)_ImageData.csv$)')[,3]
head(plateName)
virusName = str_match(layoutFile, '(^.*/BafA_Tit2_p(.+)_v(.+)_d(.+).csv$)')[,4]
head(virusName)
virusDilution = str_match(layoutFile, '(^.*/BafA_Tit2_p(.+)_v(.+)_d(.+?).csv$)')[,5]
head(virusDilution)
establishmentPlateNumber = str_match(layoutFile, '(^.*/BafA_p(.+)_v(.+)_d(.+).csv$)')[,3]
head(establishmentPlateNumber)
imagingPlateNumber = str_match(plaqueOutputFile, '(^.*/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_(.+)_ImageData.csv$)')[,3]
head(imagingPlateNumber)

# Output setup
plotTitle = 'BafA: HRV-16-GFP (1:24,000), 2 dpi on HOG'
outputPlotDirectory <- 'N:/antivir_screen/6-prestwick/6-12_HRV_confirmation/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_24/Results/postprocessed/Graphs/' 
outputPlotNameBase <- paste(virusName, virusDilution, sep="_")
outputAnalysisDirectory <- 'N:/antivir_screen/6-prestwick/6-12_HRV_confirmation/170619-RV-BafA1-non-frozen-Tit1-p2_Plate_24/Results/postprocessed/'
...
plaquesDFT_40nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='40', 'numberOfPlaques'])
plaquesDFT_40nM_mean_normalized = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='40', 'normRelPlaques'])

normInfectionIndexDFT_40nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='40', 'normRelInfected'])
normNumberOfNucleiDFT_40nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='40', 'normRelNuclei'])
...
ggplot(plaqueOut_infected, aes(x=conc, y=relInfected))+
  # There is a bug in CalcFudgeAxis leading to 7.5x right y axis values therefore /7.5
  geom_boxplot(aes(x=conc, y=fudgeTotal/7.5), col='blue', width=0.4, outlier.colour = NA)+
