...
# User input
plaqueOutputFile <- 'N:/antivir_screen/6-prestwick/6-10_HRV-BAfAEPFL/20170528-BafA-p1_Plate_4151/Results/20170528-BafA-p1_Plate_4151_ImageData.csv'
layoutFile <- 'N:/antivir_screen/6-prestwick/6-10_HRV-BAfAEPFL/20170528-BafA-p1_Plate_4151/Layouts/BafA_p1_vHRV-GFP_d1e5.csv'
...
imagingPlateNumber = str_match(plaqueOutputFile, '(^.*/20170528-BafA-p1_Plate_(.+)_ImageData.csv$)')[,3]
head(imagingPlateNumber)

# Output setup
plotTitle = 'BafA: HRV-16-GFP (1:100,000), 2 dpi on HOG'
outputPlotDirectory <- 'N:/antivir_screen/6-prestwick/6-10_HRV-BAfAEPFL/20170528-BafA-p1_Plate_4151/Results/postprocessed/Graphs/' 
outputPlotNameBase <- paste(virusName, virusDilution, sep="_")
outputAnalysisDirectory <- 'N:/antivir_screen/6-prestwick/6-10_HRV-BAfAEPFL/20170528-BafA-p1_Plate_4151/Results/postprocessed/'
...
ggplot(plaqueOut_infected, aes(x=conc, y=relInfected))+
  # There is a bug in CalcFudgeAxis leading to 15x right y axis values therefore /4
  geom_boxplot(aes(x=conc, y=fudgeTotal/15), col='blue', width=0.4, outlier.colour = NA)+
...
ggplot(plaqueOut, aes(x=conc, y=numberOfPlaques))+
  # There is a bug in CalcFudgeAxis leading to 10x right y axis values therefore /10
  geom_boxplot(aes(x=conc, y=fudgeTotal/10), col='blue', width=0.4, outlier.colour = NA)+
  geom_point(position=position_jitter(width=0.2, height=0), size=0.2, col = '#6BB100')+
  facet_grid(virus~drug, scales='free_x', space = 'free_x')+
