...
### Bug:
### CalcFudge Fudgetotal is too high, infection index max is 0.7 instead of 0.2
...
# User input
plaqueOutputFile <- 'N:/antivir_screen/6-prestwick/6-16_HRV_preZ/170828-preZ-p9-HRV_Plate_674/Results/170828-preZ-p9-HRV_Plate_674_ImageData.csv'
layoutFile <- 'N:/antivir_screen/6-prestwick/6-16_HRV_preZ/170828-preZ-p9-HRV_Plate_674/Layouts/preZ_p9_vHRV_d1e5.csv'
plateName = str_match(plaqueOutputFile, '(^.*/(.+)_ImageData.csv$)')[,3]
head(plateName)
virusName = str_match(layoutFile, '(^.*/preZ_p(.+)_v(.+)_d(.+).csv$)')[,4]
head(virusName)
virusDilution = str_match(layoutFile, '(^.*/preZ_p(.+)_v(.+)_d(.+?).csv$)')[,5]
head(virusDilution)
establishmentPlateNumber = str_match(layoutFile, '(^.*/preZ_p(.+)_v(.+)_d(.+).csv$)')[,3]
head(establishmentPlateNumber)
imagingPlateNumber = str_match(plaqueOutputFile, '(^.*/170828-preZ-p9-HRV_Plate_(.+)_ImageData.csv$)')[,3]
head(imagingPlateNumber)

# Output setup
plotTitle = 'BafA: HRV-16-GFP (1:100,000), 2 dpi on HOG'
outputPlotDirectory <- 'N:/antivir_screen/6-prestwick/6-16_HRV_preZ/170828-preZ-p9-HRV_Plate_674/Results/postprocessed/Graphs/' 
outputPlotNameBase <- paste(virusName, virusDilution, sep="_")
outputAnalysisDirectory <- 'N:/antivir_screen/6-prestwick/6-16_HRV_preZ/170828-preZ-p9-HRV_Plate_674/Results/postprocessed/'
...
plaqueOut$drug <- factor(plaqueOut$drug, levels=c('dmso'))
levels(plaqueOut$drug) <- c('DMSO')
...
plaquesDFT_61nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='061', 'numberOfPlaques'])
plaquesDFT_61nM_mean_normalized = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='061', 'normRelPlaques'])

normInfectionIndexDFT_61nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='061', 'normRelInfected'])
normNumberOfNucleiDFT_61nM_mean = mean(plaqueOut_infected[plaqueOut_infected$drug=='BafA' & plaqueOut_infected$conc=='061', 'normRelNuclei'])
...
ggplot(plaqueOut_infected, aes(x=conc, y=relInfected))+
  # There is a bug in CalcFudgeAxis leading to 1.3x right y axis values therefore /1.3
  geom_boxplot(aes(x=conc, y=fudgeTotal/1.3), col='blue', width=0.4, outlier.colour = NA)+
  geom_point(position=position_jitter(width=0.2, height=0), size=0.2, col = '#6BB100')+
  facet_grid(.~drug, scales='free_x', space = 'free_x')+
  scale_y_continuous(sec.axis=sec_axis(~.*max(plaqueOut_infected$normRelNuclei)/max(plaqueOut_infected$fudgeTotal), name='Normalized Number of Nuclei\n'), name='Infection Index\n')+
...
ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_InfInd.png', sep=''), width=5, height=10, units='cm', dpi=1200)
...
ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_Nuclei_non-inf.png', sep=''), width=5, height=10, units='cm', dpi=1200)
...
ggplot(plaqueOut, aes(x=conc, y=relInfected))+
  ...
  scale_y_continuous(sec.axis=sec_axis(~.*max(plaqueOut$numberOfNuclei)/max(plaqueOut$fudgeTotal), name='Cell Count'), name='Infection Index\n')+ ####### /max(plaqueOut$fudgeTotal)
  #scale_y_continuous(sec.axis=sec_axis(limits = c(0, max(plaqueOut$relInfected))))+
  geom_errorbar(stat='summary', fun.y='mean', width=0.6, aes(ymax=..y..,ymin=..y..), position=position_dodge(width=1.2), size=1, col='#006600')+
  stat_summary(fun.data= mean_se, geom= 'errorbar', width=0.4, size=0.5, colour='#006600')+
  ylab('Infection Index\n')+
  xlab('\nConcentration [nM]')+
...
ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_InfInd_sep.png', sep=''), width=5, height=10, units='cm', dpi=1200)
...
ggplot(plaqueOut_infected, aes(x=conc, y=normRelInfected))+
  ...
  geom_point(position=position_jitter(width=0.2, height=0), size=0.2, col = '#6BB100')+
  facet_grid(.~drug, scales='free_x', space = 'free_x')+
  scale_y_continuous(sec.axis=sec_axis(~.*max(plaqueOut_infected$normRelNuclei)/max(plaqueOut_infected$fudgeTotal), name='Cell Count (Normalized)\n'), name='Infection Index (Normalized)\n')+
  geom_errorbar(stat='summary', fun.y='mean', width=0.6, aes(ymax=..y..,ymin=..y..), position=position_dodge(width=1.2), size=1, col='#006600')+
  stat_summary(fun.data= mean_se, geom= 'errorbar', width=0.4, size=0.5, colour='#006600')+
  xlab('\nConcentration [nM]')+
  theme_bw()+
  ...
  theme(strip.text=element_text(angle=90, vjust = 0))+
  theme(strip.placement = 'outside')
ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_InfInd_norm.png', sep=''), width=5, height=10, units='cm', dpi=1200)
...
ggplot(plaqueOut, aes(x=conc, y=numberOfPlaques))+
  ...
  geom_point(position=position_jitter(width=0.2, height=0), size=0.2, col = '#6BB100')+
  facet_grid(virus~drug, scales='free_x', space = 'free_x')+
  scale_y_continuous(sec.axis=sec_axis(~.*max(plaqueOut$numberOfNuclei)/max(plaqueOut$fudgeTotal), name='Cell Count'), name='Plaque Number\n')+
  geom_errorbar(stat='summary', fun.y='mean', width=0.6, aes(ymax=..y..,ymin=..y..), position=position_dodge(width=1.2), size=1, col='#006600')+
  stat_summary(fun.data= mean_se, geom= 'errorbar', width=0.4, size=0.5, colour='#006600')+
  xlab('\nConcentration [nM]')+
  theme_bw()+
  ...
  theme(strip.text=element_text(angle=90, vjust = 0.5))+
  theme(strip.placement = 'outside')
ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_Pleaque_sep.png', sep=''), width=5, height=10, units='cm', dpi=1200)
  ...
  ggplot(plaqueOut_infected, aes(x=conc, y=numberOfPlaques))+
    ...
    facet_grid(.~drug, scales='free_x', space = 'free_x')+
    scale_y_continuous(sec.axis=sec_axis(~.*max(plaqueOut_infected$numberOfNuclei)/max(plaqueOut_infected$fudgeTotal), name='Cell Count\n'), name='Plaque Number\n')+
    geom_errorbar(stat='summary', fun.y='mean', width=0.6, aes(ymax=..y..,ymin=..y..), position=position_dodge(width=1.2), size=1, col='#006600')+
    stat_summary(fun.data= mean_se, geom= 'errorbar', width=0.4, size=0.5, colour='#006600')+
    xlab('\nConcentration [nM]')+
    theme_bw()+
    ...
    theme(strip.text=element_text(angle=90, vjust = 0))+
    theme(strip.placement = 'outside')
  ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_Plaque.png', sep=''), width=5, height=10, units='cm', dpi=1200)
  ...
  ggplot(plaqueOut_infected, aes(x=conc, y=normRelPlaques))+
    ...
    theme(strip.text=element_text(angle=90, vjust = 0))+
    theme(strip.placement = 'outside')
  ggsave(paste(outputPlotDirectory, outputPlotNameBase, '_Plaque_norm.png', sep=''), width=5, height=10, units='cm', dpi=1200)
