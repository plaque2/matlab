...
# User input
plaqueOutputFile <- 'N:/antivir_screen/6-prestwick/6-19-24_HRV_Z_a-b/Results/170914-6-19-HRV-pZ-a_Plate_801_ImageData.csv'
layoutFile <- 'N:/antivir_screen/6-prestwick/6-19-24_HRV_Z_a-b/Parameters/Za_p5_sBSF018108_vHRV_d5e5.csv'
plateName = str_match(plaqueOutputFile, '(^.*/(.+)_ImageData.csv$)')[,3]
head(plateName)
virusName = str_match(layoutFile, '(^.*/Za_p(.+)_s(.+)_v(.+)_d(.+).csv$)')[,5]
head(virusName)
virusDilution = str_match(layoutFile, '(^.*/Za_p(.+)_s(.+)_v(.+)_d(.+).csv$)')[,6]
head(virusDilution)
establishmentPlateNumber = str_match(layoutFile, '(^.*/Za_p(.+)_s(.+)_v(.+)_d(.+).csv$)')[,3]
head(establishmentPlateNumber)
imagingPlateNumber = str_match(plaqueOutputFile, '(^.*/170914-6-19-HRV-pZ-a_Plate_(.+)_ImageData.csv$)')[,3]
head(imagingPlateNumber)
zPlateNumber = str_match(layoutFile, '(^.*/Z(.+)_p(.+)_s(.+)_v(.+)_d(.+).csv$)')[,3]
head(zPlateNumber)

# Output setup
plotTitle = '\n BafA: HRV-16-GFP (1:500,000),\n 2 dpi on HOG'
