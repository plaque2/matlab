...
# User input
plaqueOutputFile <- 'N:/antivir_screen/6-prestwick/6-20-22_HAdV_Z_a-b/Results/170914-6-20-HAdV-pZ-a_Plate_802_ImageData.csv'
layoutFile <- 'N:/antivir_screen/6-prestwick/6-20-22_HAdV_Z_a-b/Parameters/Za_p1_sBSF018104_vHAdV_d5e5.csv'
...
# Output setup
plotTitle = '\n DFT: HAdV-C3-dE3B-GFP\n (1:500,000),\n 3 dpi on A549 ATCC'
outputPlotDirectory <- 'N:/antivir_screen/6-prestwick/6-20-22_HAdV_Z_a-b/Results/postprocessed/Graphs/' 
outputPlotNameBase <- paste(virusName, zPlateNumber, virusDilution, sep="_")
outputAnalysisDirectory <- 'N:/antivir_screen/6-prestwick/6-20-22_HAdV_Z_a-b/Results/postprocessed/'
...
plaqueOut$drug <- factor(plaqueOut$drug, levels=c('dmso', 'dft'))
levels(plaqueOut$drug) <- c('DMSO', 'DFT')
...
# = - 1196848609
plaqueOut$totalVirusIntensityBackgroundSubstracted = plaqueOut$totalVirusIntensity / 957113771.7 #(threshold)
...
## Calculate DFT ctr. means + STD
DFTNucleiMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'numberOfNuclei'])
DFTInfectedNucleiMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'numberOfInfectedNuclei'])
DFTInfectioIndexMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'infectionIndex'])
DFTInfectioIndexInverseMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'infectionIndexInverse'])
DFTPlaqueMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'numberOfPlaques'])
DFTGfpMean = mean(plaqueOut[plaqueOut$drug == 'DFT' , 'totalVirusIntensity'])
...
## Calculate DFT effect compared to DMSO
DFTNucleiRelative = DFTNucleiMean / DMSONucleiMean
DFTINfectedNucleiRelative = DFTInfectedNucleiMean / DMSOInfectedNucleiMean
DFTInfectionIndexRelative = DFTInfectioIndexMean / DMSOInfectioIndexMean
DFTPlaqueRelative = DFTPlaqueMean / DMSOPlaqueMean

##Z Factors
zFactorInfectionIndex = zFactor(plaqueOut$infectionIndex, "DMSO", "DFT", 0)
head(zFactorInfectionIndex)
zFactorInfectionIndexInverse =zFactor(plaqueOut$infectionIndexInverse, "DMSO", "DFT", 0)
head(zFactorInfectionIndexInverse)
zFactorNumberOfInfectedNuclei = zFactor(plaqueOut$numberOfInfectedNuclei, "DMSO", "DFT", 0)
head(zFactorNumberOfInfectedNuclei)
zFactorGfpIntensity = zFactor(plaqueOut$totalVirusIntensity, "DMSO", "DFT", 0)
head(zFactorGfpIntensity)
zFactorGfpBackIntensity = zFactor(plaqueOut$totalVirusIntensity, "DMSO", "DFT", 957113771.7)
head(zFactorGfpBackIntensity)
zFactorGfpLogIntensity = zFactor(plaqueOut$totalVirusIntensityLog, "DMSO", "DFT", 0)
head(zFactorGfpLogIntensity)
zFactorGfpMeanIntensity = zFactor(plaqueOut$meanVirusIntensity, "DMSO", "DFT", 0)
head(zFactorGfpMeanIntensity)
zFactorNumberOfPlaques =  zFactor(plaqueOut$numberOfPlaques, "DMSO", "DFT", 0)
head(zFactorNumberOfPlaques)
zFactorNumberOfNuclei =zFactor(plaqueOut$numberOfNuclei, "DMSO", "DFT", 0)
